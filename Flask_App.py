...
def after_token():
    ...
    if pres is not None:
        body += "<p>{0} prescriptions: <ul><li>{1}</li></ul></p>".format(
            "His" if 'male' == smart.patient.gender else "Her", '</li><li>'.join([_med_name(p) for p in pres]))
    else:
...
def after_token2():
    ...
    vitals = [result[0].resource for result in
...
def bpsubmit():
    if(request.method=='POST'):
        ...
        with open('/Users/anushreesinha/Documents/dicto.csv', 'w') as csvfile:
...
@app.route('/getfordoc')
def getfordoc():
    with open('/Users/anushreesinha/Documents/dicto.csv') as csvfile:
